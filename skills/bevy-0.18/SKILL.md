---
name: bevy-0.18
description: This skill should be used when working on Bevy 0.18 game engine projects. It provides specialized knowledge for Bevy's Entity Component System (ECS) architecture, component-driven design patterns, system ordering, UI development, build strategies, and common pitfalls specific to Bevy 0.18. Use this skill when implementing game features, debugging Bevy code, designing component architectures, or working with Bevy's UI system.
---

# Bevy 0.18 Game Development Skill

A specialized skill for developing games and applications using the Bevy 0.18 game engine, incorporating all breaking changes and new APIs introduced in the 0.17-to-0.18 migration.

## When to Use This Skill

Invoke this skill when:
- Implementing features in a Bevy 0.18 game or application
...
- Migrating a project from Bevy 0.17 to 0.18
...
- `RenderTarget` is now a standalone component (not a field on `Camera`)
- `AmbientLight` split into `AmbientLight` (component on Camera) and `GlobalAmbientLight` (resource)
- State transitions: `set()` now always triggers `OnEnter`/`OnExit`; use `set_if_neq()` for old behavior
- `EntityEvent` is immutable by default; use `SetEntityEventTarget` trait for mutable access
- `Internal` component removed — observer/one-shot system entities no longer hidden
- `Mesh::try_*` functions required when `RenderAssetUsages::RENDER_WORLD`-only meshes may exist
- Feature renames: `animation` -> `gltf_animation`, picking backends renamed
- Input sources (`mouse`, `keyboard`, `gamepad`, `touch`, `gestures`) now behind feature flags
- `Commands::get_entity` no longer errors for non-spawned entities; use `Commands::get_spawned_entity` for spawned-only
- `#[reflect(...)]` only supports parentheses (not braces or brackets)

**See `references/bevy_specific_tips.md` for complete Bevy 0.18 details and examples.**
...
**Note (0.18):** Only the text-containing areas of `Text` UI nodes are pickable. Non-text areas do not register pointer hits. To intercept hits on the full node area, use an intermediate parent node.
...
1. State transitions now always fire `OnEnter`/`OnExit` — use `set_if_neq()` to preserve old behavior
2. `RenderTarget` is a component, not a `Camera` field
3. Forgetting to register systems in `main.rs`
4. Borrowing conflicts (use `get_many_mut` for multiple mutations)
5. Not using `Changed<T>` for expensive operations
6. Wrong system ordering (input -> state -> derived -> visual -> UI)
7. Entity queries after despawn (use `if let Ok()` pattern)
8. Material/asset handle confusion (store handles properly)
...
- `references/bevy_specific_tips.md` - **START HERE:** Registry examples, plugin structure, build optimization, version management, domain-driven design for ECS, and all Bevy 0.18 API changes
...
- `references/common_pitfalls.md` - Common mistakes and their solutions (state transitions, RenderTarget, system registration, borrowing conflicts, change detection, system ordering, entity queries, asset handles)
...
- Migration Guide 0.17 to 0.18: https://bevy.org/learn/migration-guides/0-17-to-0-18/
